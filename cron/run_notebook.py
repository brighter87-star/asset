...
주말 및 한국 공휴일(비거래일)은 자동으로 건너뜁니다.
market_index 테이블에 오늘 데이터가 없으면 비거래일로 판단합니다.
(daily_sync.py가 market_index를 동기화하므로 이 스크립트는 그 이후에 실행해야 함)
...
def main():
    ...
    print(f"[run_notebook] {today} (KST)")

    if not args.force:
        # 1. 주말 skip
        if today.weekday() >= 5:
            print(f"[SKIP] 주말 ({today}). 노트북 실행 안 함.")
            return
        ...
        if count == 0:
            print(f"[SKIP] 비거래일 ({today}). market_index 없음.")
            return
    ...
    if result.returncode == 0:
        print("[OK] 노트북 실행 완료. 이미지 저장됨: notebooks/images/")
    else:
